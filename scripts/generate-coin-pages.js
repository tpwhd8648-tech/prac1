  ...
  <script>
    ...
    async function loadPriceAndStock() {
      try {
        ...
        if (tbGold && tbRate) {
          const goldText = tbGold.textContent.replace('$', '').trim();
          const rateText = tbRate.textContent.replace('원', '').replace(/,/g, '').trim();
          const gold = parseFloat(goldText);
          const rate = parseFloat(rateText);
          if (gold > 0 && rate > 0) {
            const krwPerOz = gold * rate;
            const price = Math.round(krwPerOz * product.premium / 1000) * 1000;
